...
func TestTransaction_HasAutoGeneratedID(t *testing.T) {
	...
	// perform test
	tx := NewTransaction(NewWallet(m, new(MockedCalculating), 1000), "receiver", 0)
...
func TestTransaction_OutputHasAmountToReceiver(t *testing.T) {
	...
	// perform test
	tx := NewTransaction(NewWallet(m, new(MockedCalculating), 1000), "receiver", 1)
...
func TestTransaction_OutputHasRemainingBalanceOfSenderWallet(t *testing.T) {
	m := crypto.NewSecp256k1Generator()
	w := NewWallet(m, new(MockedCalculating), 1000)
...
func TestTransaction_Append(t *testing.T) {
	...
	secp256k1 := crypto.NewSecp256k1Generator()
	senderWallet := NewWallet(secp256k1, new(MockedCalculating), 1000)
	receiverAWallet := NewWallet(secp256k1, new(MockedCalculating), 1000)
	receiverBWallet := NewWallet(secp256k1, new(MockedCalculating), 1000)
...
func TestCreateRewardTransaction(t *testing.T) {
	...
	beforeEach := func() {
		minerWallet = NewWallet(secp256k1, new(MockedCalculating), 1000)
