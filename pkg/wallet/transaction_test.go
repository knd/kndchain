...
func TestTransaction_HasAutoGeneratedID(t *testing.T) {
	...
	// perform test
	tx := NewTransaction(NewWallet(m, new(MockedCalculating)), "receiver", 0)
...
func TestTransaction_OutputHasAmountToReceiver(t *testing.T) {
	...
	// perform test
	tx := NewTransaction(NewWallet(m, new(MockedCalculating)), "receiver", 1)
...
func TestTransaction_OutputHasRemainingBalanceOfSenderWallet(t *testing.T) {
	m := crypto.NewSecp256k1Generator()
	w := NewWallet(m, new(MockedCalculating))
...
func TestTransaction_Input(t *testing.T) {
	...
	t.Run("sets `amount` to the `senderWallet` balance", func(t *testing.T) {
		assert.Equal(config.InitialBalance, tx.GetInput().Amount)
	})
...
func TestTransaction_Append(t *testing.T) {
	...
	secp256k1 := crypto.NewSecp256k1Generator()
	senderWallet := NewWallet(secp256k1, new(MockedCalculating))
	receiverAWallet := NewWallet(secp256k1, new(MockedCalculating))
	receiverBWallet := NewWallet(secp256k1, new(MockedCalculating))
...
func TestCreateRewardTransaction(t *testing.T) {
	...
	beforeEach := func() {
		minerWallet = NewWallet(secp256k1, new(MockedCalculating))
