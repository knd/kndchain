...
func TestService_ContainsValidTransactions(t *testing.T) {
	...
	createBlock := func(timestamp int64, lastHash *string, hash *string, data []Transaction, nonce uint32, difficulty uint32) Block {
		return Block{
			Timestamp:  timestamp,
			LastHash:   lastHash,
			Hash:       hash,
	...
	toListingBlockchain := func(bc *Blockchain) *listing.Blockchain {
		if bc == nil {
			return nil
		}

		result := &listing.Blockchain{}
		for _, block := range bc.Chain {
			cTransactions := []listing.Transaction{}
			for _, transaction := range block.Data {
				cTx := listing.Transaction{
					ID:     transaction.ID,
	...
	t.Run("returns true if blockchain contains all valid transactions", func(t *testing.T) {
		...
		block := createBlock(blockTs.UnixNano(), &lastHash, &hash, data, nonce, difficulty)
